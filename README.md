<h3 align="center">
  ...
  <p align="center"><img src="https://img.shields.io/badge/WLCM%20TO -ArYAN NIX DOWNLOADER-green?colorA=%23ff0000&colorB=%23017e40&style=flat-square">  
  
</h3>

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Neuton&size=25&color=30FF40&background=000000&center=true&vCenter=true&width=360&height=60&lines=Hello+World%2C+I'm+ArYAN+Here+🤙;𝙸𝚃'𝚜+𝙽𝙾𝚃+𝙰+𝙹𝚄𝚂𝚃+𝙽𝙰𝙼𝙴+𝙱𝚁𝙾+🥱;𝙸𝚃'𝚜+𝙰+𝙱𝚁𝙰𝙽𝙳+🔥;Respect+ArYAN+🥀;Thanks+My+All+Friend+🤙+🥰)](https://git.io/typing-svg)



## Instalation :
```bash
> npm i aryan-nix-downloader
```
## 🚀 Project Update Log  
...
## Example (fb & insta)
```js
const { ndown } = require("aryan-nix-downloader")
...
{
    "developer": "itz Aryan",
    "devfb": "https://www.facebook.com/profile.php?id=100001200784032",
...
## Instagram Usage 
```js
const {instagram} = require("aryan-nix-downloader");
...
## Example (TikTok)
```js
const { tikdown } = require("aryan-nix-downloader")
...
## Example (YouTube)
```js
const { ytdown } = require("-media-downloaders")
let URL = await ytdown("https://youtu.be/aRSuyrZFu_Q?si=bsfzgeeGmRpsHqnF")
console.log(URL)
```
## Example (Twitter)
```js
const { twitterdown } = require("aryan-nix-downloader")
...
## Example Fbdown2
```js
const { fbdown2 } = require("aryan-nix-downloader");
...
## Usage GDLink 
```js
const {GDLink} = require("aryan-nix-downloader");
...
## Usage Pintarest 
```js
const {pintarest} = require("aryan-nix-downloader");
...
## Usage CapCut 
```js
const { capcut } = require("aryan-nix-downloader");
...
## Usage Likee 
```js
const { likee} = require("aryan-nix-downloader");
...
## Usage Threads
```js
const { threads } = require("aryan-nix-downloader");
...
```js
(async () => {
  const { spotifySearch } = require("aryan-nix-downloader");
...
```js
(async () => {
  const { spotifyDl } = require("aryan-nix-downloader");
...
```js
(async () => {
  const { soundcloudSearch } = require("aryan-nix-downloader");
...
```js
(async () => {
  const { soundcloud } = require("aryan-nix-downloader");
...
```js
(async () => {
  const { terabox } = require("aryan-nix-downloader");
...
```
```js
const {alldown} = require("aryan-nix-downloader");
...
[![Facebook](https://img.shields.io/badge/Facebook-green?style=for-the-badge&logo=facebook)](https://www.facebook.com/profile.php?id=100001200784032)
[![Messenger](https://img.shields.io/badge/Chat-Messenger-blue?style=for-the-badge&logo=messenger)](https://m.me/100001200784032)
[![Github](https://img.shields.io/badge/Github-MrDarkYTgreen?style=for-the-badge&logo=github)](https://github.com/RFS-ARYAN)
